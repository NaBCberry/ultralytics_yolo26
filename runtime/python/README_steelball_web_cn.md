...
- `--mjpeg-passthrough`：默认启用。实时与拍摄页面均直接推送摄像头原始 MJPG；实时识别框由浏览器叠加，避免服务端重新 JPEG 编码。不支持时服务会自动回退并输出警告日志。
- `--stream-width`：MJPG 直通不可用时的网页预览宽度，默认 `640`，用于降低 JPEG 编码负载；拍摄照片和模型输入仍为摄像头原始分辨率。
...
- `--log-level DEBUG`：输出预处理、BPU 前向和后处理的单帧耗时，用于定位瓶颈；正常运行保持默认 `INFO`，避免逐帧日志影响帧率。
- `--timing-interval`：默认每 60 帧在服务端终端输出一次完整处理链路的平均耗时；设为更大值可进一步降低日志频率。
